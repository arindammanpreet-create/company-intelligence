...
Auto-generates research gaps, competitor intelligence, and ESG monitoring
...
def process_company(company_name, config):
    ...
    # 6. Generate MR&I Solutions based on gaps
...
def generate_solutions(gaps, company_name):
    """Generate MR&I solutions based on detected gaps"""
    solutions = []
    ...
    for i, gap in enumerate(gaps):
        ...
        solution_templates = {
            "revenue_risk": {
                ...
                "description": f"Primary research among 5,000+ customers and channel partners to map demand drivers and pricing sensitivity. Quarterly revenue risk dashboard with early warning indicators for {company_name} product lines."
            },
            "margin_pressure": {
                "title": f"{company_name} Cost Structure & Pricing Power Analysis",
                "description": f"Deep-dive margin decomposition across business units. Benchmark input costs and pricing vs. competitors. Monthly margin intelligence reports with actionable pricing recommendations for {company_name}."
            },
            "competition": {
                "title": f"Competitive Positioning Intelligence for {company_name}",
                "description": f"Continuous competitive monitoring across {company_name}'s key rivals. Track product launches, pricing moves, and market share shifts. Weekly competitive battlecards and win/loss analysis for sales teams."
            },
            "regulatory": {
                "title": f"Policy & Regulatory Risk Monitoring for {company_name}",
                "description": f"Track SEBI, RBI, TRAI, and ministry circulars affecting {company_name}. Model compliance cost scenarios. Monthly regulatory horizon scanning with impact assessment on revenue and operations."
            },
            "talent": {
                "title": f"Talent Market Intelligence & Employer Brand Research for {company_name}",
                "description": f"Quarterly talent supply-demand analysis for {company_name} key roles. Benchmark compensation and attrition vs. peers. Employee sentiment tracking via Glassdoor, LinkedIn, and internal surveys."
            },
            "debt_leverage": {
                "title": f"Credit Risk & Capital Structure Intelligence for {company_name}",
                "description": f"Monitor {company_name} debt covenants, credit rating outlook, and refinancing pipeline. Quarterly capital market intelligence with investor perception tracking and optimal capital structure modeling."
            },
            "esg_concern": {
                "title": f"ESG Performance Benchmarking & Stakeholder Perception for {company_name}",
                "description": f"Comparative ESG scoring across {company_name} and sector peers. Track BRSR disclosures, carbon trajectory, and controversy alerts. Quarterly sustainability perception index among investors and consumers."
            },
            "digital_lag": {
                "title": f"Digital Maturity Assessment & Tech Investment ROI for {company_name}",
                "description": f"Benchmark {company_name} digital capabilities vs. industry leaders. Survey 500+ executives on technology adoption barriers. Quarterly digital transformation progress tracker with investment prioritization matrix."
            },
            "geopolitical": {
                "title": f"Supply Chain Resilience & Geographic Risk Intelligence for {company_name}",
                "description": f"Map {company_name} supplier concentration and geographic exposure. Model geopolitical scenario impacts. Quarterly supply chain risk dashboard with alternative sourcing recommendations."
            },
            "consumer_sentiment": {
                "title": f"Brand Health Tracking & Reputation Risk Monitoring for {company_name}",
                "description": f"Continuous brand perception tracking across social media, news, and consumer panels. Sentiment trend analysis with crisis early warning. Monthly brand health scorecard with competitive benchmarking."
            },
            "monitoring": {
                "title": f"Strategic Market Intelligence Subscription for {company_name}",
                "description": f"Ongoing market monitoring covering sector trends, competitor moves, and macro indicators for {company_name}. Quarterly strategy briefings with scenario planning and opportunity identification."
            }
        }
        ...
        timeline = "6-12 months" if severity == "High" else "12-18 months"
